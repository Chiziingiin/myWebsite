...
使用Index DB技术，高效地在前端实现“数据库”，软件分为首页面，归档页面，设置页面（回收站，暗色模式，使用说明等）；
...
1. 在首页添加一个“进度”时，点击“+”号即可增加进度。当进度超过15%时，进度文字会从进度条外部运动到内部，以确保进度数字能够被清晰读取，这个设计既能巧妙避免进度条较短时导致进度文字显示不全等问题，又增加了动画效果，提升用户使用软件的趣味性；

2. 首页的日历下方有一个下三角按钮，点击即可将周视图切换为月视图，过渡动画和背景遮罩顺畅自然； 


3. 首页搜索框支持模糊搜索，例如输入“1”即可匹配到“21”、“13”、“515”等标题；


4. 个性化：软件自开发初期就尽力适配各种情况，各个条目均支持设置主题色，并提供暗色模式设置，充分考虑用户的个性化需求；


5. 为了使软件更具有可维护性，在设计主题时，编写样式文件的语言选择了SASS，并且分文件模块化编写，利用SASS与编译器的语言优势，降低代码后续的维护成本；
